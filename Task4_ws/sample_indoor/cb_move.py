...
def set_velocity(w_l, w_r, w_l_r, w_r_r):
    ...
    ser.write(str([w_l_r, w_r_r+20]))

def move_robot():
    global goal_pos
    pos_on_path = 0
    distance = 0
    ret = path_from_start_to_goal()
    global rob_pos
    while(1):
	...
	wheel_seperation_r = 0.18

        v_r = v_des + (wheel_seperation/2) * w_des
        v_l = v_des - (wheel_seperation/2) * w_des

	v_r_r = v_des_r + (wheel_seperation_r/2) * w_des_r
        v_l_r = v_des_r - (wheel_seperation_r/2) * w_des_r
	...
	w_r_r = int(v_r_r/wheel_radius*40)
        w_l_r = int(v_l_r/wheel_radius*40)

	print w_l_r, w_r_r
	print w_r, w_l
	if w_r_r < 80 and w_r_r > 0:
		w_r_r = 80
	if w_l_r < 80 and w_l_r > 0:
		w_r_r = 80
	if w_r_r > -80 and w_r_r < 0:
		w_r_r = -80
	if w_l_r > -80 and w_l_r < 0:
	...
	out=''
        ...
        set_velocity(w_l, w_r, w_l_r, w_r_r)
...
flag = 0
l = 0.864 #arena length in meters of the actual arena
w = 0.889 #arena width in meters of the actual arena
top_left_w = 0
top_left_h = 0
cap.set(3, 1280)
cap.set(4, 720)
